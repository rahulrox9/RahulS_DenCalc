...
def Density(H2O_fit_params):
    ...
    def calculate_mineral_density(xl, Density):
        ...
        xl['Ab'] = np.where(xl['Phase']=='Plagioclase',(xl['Na2O']/61.979)/((xl['CaO']/56.0774)+2*(xl['Na2O']/61.979)+2*(xl['K2O']/94.196)),np.nan)
        xl['Or'] = np.where(xl['Phase']=='Plagioclase',(xl['K2O']/94.196)/((xl['CaO']/56.0774)+2*(xl['Na2O']/61.979)+2*(xl['K2O']/94.196)),np.nan)
    ...
    propn = pd.read_csv(os.path.join(data_folder,'Norm_Propn.csv'), index_col=0)
